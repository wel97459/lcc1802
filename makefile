...
# 15-01-09 adding XR18CX for LCC1802
# 20-05-10 XR18CX replaces XR19DH
# 20-05-11 adding back lines to keep xr18DH and CX
A=.a
O=.o
E=
CC=cc
...
B=$(BUILDDIR)
T=$(TSTDIR)
HOSTFILE=etc/linux.c #wjr
TARGET=x86/linux #wjr
BUILDDIR=build/
LCCDIR=build/

all::	rcc lburg cpp lcc bprint liblcc copt

what:
	-@echo make all rcc lburg cpp copt lcc bprint liblcc triple clean clobber
...
$Bxr18DH$O:	$Bxr18DH.c;	$(CC) $(CFLAGS) -c -Isrc -o$@ $Bxr18DH.c
$Bxr18CX$O:	$Bxr18CX.c;	$(CC) $(CFLAGS) -c -Isrc -o$@ $Bxr18CX.c
...
$Bxr18DH.c:	$Blburg$E src/xr18DH.md;   $Blburg src/xr18DH.md       $@
$Bxr18CX.c:	$Blburg$E src/xr18CX.md;   $Blburg src/xr18CX.md       $@
...
COPTOBJS=$Bcopt$O
$Bcopt$E:		$(COPTOBJS)
				$(LD) $(LDFLAGS) -o $@ $(COPTOBJS)
$(COPTOBJS):	copt/copt.h
$Bcopt$O:		copt/copt.c;	$(CC) $(CFLAGS) -c -Icopt -o$@ copt/copt.c
...
clobber::	clean
		$(RM) $Brcc$E $Blburg$E $Bcpp$E $Bcopt$E $Blcc$E $Bcp$E $Bbprint$E $B*$A
		$(RM) $B*.pdb $B*.pch
...
install:
	@mkdir -p bin
	@cd build; cp rcc lburg cpp copt lcc bprint liblcc.a librcc.a ../bin/
